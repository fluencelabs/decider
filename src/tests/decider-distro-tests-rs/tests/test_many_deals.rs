...
async fn test_many() {
    ...
    run_decider(&mut server, &mut client, chain_replies).await;
    {
        ...
        for joined_deal in joined.iter() {
            ...
            assert!(
                !worker_id.is_empty(),
                "Worker for deal {} must be created",
                joined_deal.deal_id
            );
            let is_active = worker::is_active(&mut client, &joined_deal.deal_id)
                .await
                .unwrap();
            assert!(
                is_active,
                "Worker for deal {} must be active",
                joined_deal.deal_id
            );
        }
    }
    ...
    run_decider(
        &mut server,
        &mut client,
        ChainReplies::new(updated_deals.clone(), vec![]),
    )
    .await;
    {
        let joined = deal::get_joined_deals(&mut client).await;
        assert_eq!(
            joined.len(),
            updated_deals.len(),
            "all deals must be installed, expected: {updated_deals:?}, got: {joined:?}"
        );
        for joined_deal in joined.iter() {
            let deal = updated_deals
                .iter()
                .find(|d| d.deal_id == joined_deal.deal_id);
            assert!(
                deal.is_some(),
                "deal {} wasn't installed",
                joined_deal.deal_id
            ...
            assert_eq!(
                app_cid,
                deal.app.as_ref().unwrap().cid,
                "Deal CID wasn't updated"
            );
        }
    }
    ...
    run_decider(&mut server, &mut client, ChainReplies::new(vec![], vec![])).await;
    {
        ...
        assert!(
            joined.is_empty(),
            "all deals must be removed, got: {joined:?}"
        );

        let worker_list = worker::get_worker_list(&mut client).await;
        assert!(
            worker_list.is_empty(),
            "all workers must be removed, got: {worker_list:?}"
        );
    }
...
async fn test_install_again() {
    ...
    let chain_replies = ChainReplies::new(deals.clone(), vec![]);
    run_decider(&mut server, &mut client, chain_replies).await;
    {
        let joined = deal::get_joined_deals(&mut client).await;
        assert_eq!(
            joined.len(),
            deals.len(),
            "deal {removed_deal:?} must be removed, got list of deals: {joined:?}"
        );
        for joined_deal in joined.iter() {
    ...
    run_decider(
        &mut server,
        &mut client,
        ChainReplies::new(deals.clone(), tx_hashes),
    )
    .await;
    {
        let joined = deal::get_joined_deals(&mut client).await;
        assert_eq!(
            joined.len(),
            deals.len(),
            "new deal {} must be installed, expected list of deals: {deals:?}, got: {joined:?}",
            DEAL_IDS[3]
        );
        for joined_deal in joined.iter() {
            let deal = deals.iter().find(|d| d.deal_id == joined_deal.deal_id);
