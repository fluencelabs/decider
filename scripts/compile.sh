...
# "with_decider": "$FLUENCE_ENV_CONNECTOR_WITH_DECIDER"
# "cfg": decider cfg
# "dat":
...
jq -s --arg script "$(cat $dir/decider.main.air | awk '{$1=$1};1')" '{ "script": $script, "with_decider": "$FLUENCE_ENV_CONNECTOR_WITH_DECIDER", "cfg": .[0], "dat": .[1]}' "$decider_config" "$dir"/init.json > decider.json
